...
const { backendHost, backendInvocation, backendPort, frontendEnvironment, frontendHost, frontendPort, readStartupTimeoutMs } = require("./dev");
...
const statusHealthTimeoutMs = 3000;
...
async function startDaemon() {
  ...
  try {
    ...
    const child = spawnNodeScript(path.join(__dirname, "..", "bin", "ornnlab.js"), ["dev", "_daemon", "--token", daemonToken], {
...
function restartDetached() {
  ensureServiceHome();
  const daemonLog = openPrivateLog(devServicePaths().logs.daemon, ensureServiceHome);
  const child = spawnNodeScript(path.join(__dirname, "..", "bin", "ornnlab.js"), ["dev", "restart"], {
    cwd: path.join(__dirname, ".."),
    detached: true,
...
async function runDaemon() {
  ...
  logEvent("dev_service.start_requested", { backendUrl, frontendUrl, startupTimeoutMs });
  ...
  try {
    ...
    runtime.frontend = await startManagedChild("frontend", `${frontendUrl}${livePath}`, runtime);
...
async function startManagedChild(name, healthUrl, runtime) {
  ...
  const child = spawnNodeScript(path.join(__dirname, "dev-child-wrapper.js"), ["--token", token, "--role", name, "--", command, ...args], {
    cwd,
...
async function onChildExit(name, code, signal, runtime) {
  ...
  const backendUrl = `http://${backendHost}:${backendPort}`;
  const frontendUrl = `http://${frontendHost}:${frontendPort}`;
  const healthUrl = name === "backend" ? backendUrl : `${frontendUrl}${livePath}`;
...
function commandFor(name) {
  ...
  if (name === "backend") {
    const backend = backendInvocation(["web", "--host", backendHost, "--port", backendPort]);
    return {
      command: backend.command,
      args: backend.args,
      cwd: sourceDir,
      env: { ...process.env, ORNNLAB_RESTART_COMMAND: formatNodeScriptCommand(path.join(__dirname, "..", "bin", "ornnlab.js"), ["dev", "_restart-detached"]) },
    };
  }
  return {
    command: "npm",
...
function writeState(state) {
  ensureServiceHome();
  const statePath = devServicePaths().state;
...
async function currentStatus() {
  ...
  const backendHealthy = backendAlive && await isHealthyEndpoint(`${state.backendUrl || `http://${backendHost}:${backendPort}`}${livePath}`);
  const frontendHealthy = frontendAlive && await isHealthyEndpoint(`${state.frontendUrl || `http://${frontendHost}:${frontendPort}`}${livePath}`);
...
async function waitForDaemonReady(child) {
  const deadline = Date.now() + startupTimeoutMs;
  while (Date.now() < deadline) {
    if (child?.ornnlabSpawnError) throw new Error(`OrnnLab dev service daemon failed to spawn: ${child.ornnlabSpawnError.message}`);
    const status = await currentStatus();
    if (status.status === "running" || status.status === "error") return;
    await sleep(250);
  }
  const state = readState();
  const logTail = tailLogFile(devServicePaths().logs.daemon, 8);
  throw new Error(`OrnnLab dev service did not become ready. lastError=${state.lastError || "none"} daemonLog=${logTail}`);
}

function tailLogFile(logPath, lines) {
  try {
...
async function waitForHealth(url, service, child) {
  const endpoint = service === "backend" ? `${url}${livePath}` : url;
  const deadline = Date.now() + startupTimeoutMs;
  while (Date.now() < deadline) {
    if (child.ornnlabSpawnError) throw new Error(`${service} failed to spawn: ${child.ornnlabSpawnError.message}`);
...
async function isHealthyEndpoint(url) {
  if (!url) return false;
  try {
    return (await fetch(url, { signal: AbortSignal.timeout(statusHealthTimeoutMs) })).ok;
  } catch {
    return false;
  }
}
...
function spawnNodeScript(scriptPath, args = [], options = {}) {
  const invocation = nodeScriptInvocation(scriptPath, args);
  const child = spawn(invocation.command, invocation.args, options);
  child.ornnlabSpawnError = null;
  child.once("error", (error) => {
    child.ornnlabSpawnError = error;
  });
  return child;
}

function formatNodeScriptCommand(scriptPath, args = []) {
  const invocation = nodeScriptInvocation(scriptPath, args);
  return [invocation.command, ...invocation.args].map(shellQuote).join(" ");
}

function nodeScriptInvocation(scriptPath, args = []) {
  const override = process.env.ORNNLAB_NODE_COMMAND;
  if (override) {
    const [command, ...nodeArgs] = splitCommand(override);
    return { command, args: [...nodeArgs, scriptPath, ...args] };
  }
  if (process.platform === "win32") return { command: "node", args: [scriptPath, ...args] };
  return { command: "/usr/bin/env", args: ["node", scriptPath, ...args] };
}
