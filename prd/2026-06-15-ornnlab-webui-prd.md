...
- Created: 2026-06-15
- Updated: 2026-06-15
- Owner / requester: OrnnLab team
- Source request: 以 WebUI 为第一优先级，重构 OrnnLab 为基于 Web 的 agent 评估平台，核心提供 agent 管理和实验管理两大模块
- Canonical engineering plan: `docs/plans/2026-06-15-harbor-webui-redesign-engineering-plan.md`

## Requester Review Summary

- Key decisions:
  - WebUI 优先，CLI 降级为辅助工具（`ornnlab web` 启动本地服务）
  - 纯本地单用户项目，无认证机制
...
- Important exceptions:
  - Harbor 原始 job artifacts 必须保留并可打开；OrnnLab 提供摘要 shell、失败解释、复用入口和原始 artifact 链接
...
OrnnLab 当前是一个 Rust CLI 工具，用于评估 CLI coding agent 在 benchmark 上的表现。产品方向调整为：**以 WebUI 为第一优先级**，让用户通过简单易用的界面快速管理 agent 配置、启动实验、查看结果和榜单排名。

底层执行引擎复用 Harbor 框架（已通过 Phase 0 评估验证），OrnnLab 专注于提供优秀的交互体验和产品语义。
...
### 使用场景
- 开发者本地运行，单机使用
- 通过 `ornnlab web` 命令启动本地 HTTP 服务，浏览器访问 `localhost:xxxx`
...
- 实验报告查看（OrnnLab 摘要 shell + Harbor 原始 artifact 链接）
- 实验删除
...
1. 用户获得报告链接或文件路径
2. 浏览器打开报告（OrnnLab HTML 摘要 shell，内含 Harbor 原始 artifact 链接）
...
- OrnnLab 生成报告摘要 shell，保留 Harbor `result.json`、`job.log`、trial artifact 的原始链接
...
- [ ] Given 实验完成，when 点击查看报告，then 展示 OrnnLab 摘要 shell 和 Harbor 原始 artifact 链接
...
- [x] 报告策略：OrnnLab 摘要 shell + Harbor 原始 artifact 链接，不使用 iframe 作为主报告体验
...
| 报告 | OrnnLab 摘要 shell + Harbor 原始 artifact 链接 | 兼顾易读摘要、失败解释和原始可追溯性 | R2 + 2026-06-15 plan refinement |
...
| 部署形态 | `ornnlab web` 本地服务 | 单机使用，localhost 访问 | R4 |
...
- **报告**：OrnnLab 摘要 shell + Harbor 原始 artifact 链接
...
- 启动方式：`ornnlab web` / `python -m ornnlab web`（FastAPI + uvicorn；开发环境使用 `uv run ornnlab web`）
