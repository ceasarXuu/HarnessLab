...
pub(super) fn terminal_bench_infra_failure(attempt_dir: &Path) -> Option<FailureCode> {
    find_log_files(attempt_dir).into_iter().find_map(|path| {
        log_contains(&path, DOCKER_NETWORK_POOL_EXHAUSTED)
            .then_some(FailureCode::DockerNetworkPoolExhausted)
    })
}
...
fn find_log_files(root: &Path) -> Vec<PathBuf> {
    ...
    while let Some(path) = pending.pop() {
        ...
        } else if is_diagnostic_log(&path) {
            logs.push(path);
        }
    }
    logs
...
fn is_diagnostic_log(path: &Path) -> bool {
    matches!(
        ...
        Some("run.log" | "stderr.log" | "stdout.log")
    )
}

fn log_contains(path: &Path, needle: &str) -> bool {
    read_log_sample(path)
...
mod tests {
    ...
    fn scans_large_log_tail_for_late_docker_network_pool_exhaustion() {
        ...
        fs::write(tmp.path().join("run.log"), content).unwrap();
