...
from harnesslab.storage.paths import ensure_parent

MANIFEST_NAME = "harnesslab-backup-manifest.json"
BACKUP_PREFIX = "harnesslab-backup"
...
class BackupService:
    ...
    def export_home(self, output: Path | None = None) -> dict[str, Any]:
        ...
        manifest = {
            "schema_version": 1,
            "harnesslab_version": __version__,
            "created_at": now_iso(),
    ...
    def _backup_files(self, archive_path: Path) -> list[Path]:
        ...
        for path in sorted(home.rglob("*")):
            ...
            if _is_under(resolved, self.settings.exports_dir.resolve()):
                continue
            if path.name in {"harnesslab.sqlite-wal", "harnesslab.sqlite-shm"}:
                continue
            files.append(path)
        return files
