...
from harnesslab.models.harbor import HarborJobConfigView
...
def _command_from_env() -> list[str]:
    raw = os.environ.get("HARNESSLAB_HARBOR_SUBPROCESS_COMMAND", "harbor run")
    command = shlex.split(raw)
    if not command:
        raise ValueError("HARNESSLAB_HARBOR_SUBPROCESS_COMMAND cannot be empty")
    return command
