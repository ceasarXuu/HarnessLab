...
describe('App', () => {
  ...
  it('shows Harbor maintenance operations in system', () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Cleanup scope' })).toBeInTheDocument()
    expect(screen.getByLabelText('Clean all (Docker + ~/.cache/harbor)')).toBeChecked()
    expect(screen.queryByText(/harbor cache clean/)).not.toBeInTheDocument()
    fireEvent.click(screen.getByLabelText('Clean Docker only'))
    expect(screen.getByLabelText('Clean all (Docker + ~/.cache/harbor)')).not.toBeChecked()
    expect(screen.getByLabelText('Clean Docker only')).toBeChecked()
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
    expect(screen.queryByRole('dialog', { name: 'Cleanup scope' })).not.toBeInTheDocument()
