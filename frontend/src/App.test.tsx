...
describe('App', () => {
  ...
  it('shows Harbor maintenance operations in system', () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Cache cleanup' })).toBeInTheDocument()
    expect(screen.getByLabelText('Preview only (--dry)')).toBeChecked()
    expect(screen.getByText('harbor cache clean --dry')).toBeInTheDocument()
    fireEvent.click(screen.getByLabelText('Skip Docker images (--no-docker)'))
    fireEvent.click(screen.getByLabelText('Skip cache directory (--no-cache-dir)'))
    expect(screen.getByText('harbor cache clean --dry --no-docker --no-cache-dir')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
    expect(screen.queryByRole('dialog', { name: 'Cache cleanup' })).not.toBeInTheDocument()
