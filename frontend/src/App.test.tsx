...
describe('App', () => {
  ...
  it('shows Harbor maintenance operations in system', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Harbor commands' })).toBeInTheDocument()
    expect(screen.getByText('Interrupted runs reconciled')).toBeInTheDocument()
    expect(screen.getByText('Authentication')).toBeInTheDocument()
    expect(screen.getByText('Cache and plugins')).toBeInTheDocument()
    ...
    expect(screen.getByText('harbor auth logout')).toBeInTheDocument()
    expect(screen.getByText('harbor cache clean --dry-run')).toBeInTheDocument()
    expect(screen.getByText('harbor sync ./dataset.toml')).toBeInTheDocument()
    expect(screen.getByText('harbor leaderboard submit job_91a7')).toBeInTheDocument()
  })
})
