...
describe('Environment templates', () => {
  ...
  it('manages OrnnLab-local templates over Harbor config fields', () => {
    ...
    expect(within(editableEnvironmentDialog).getByLabelText('type').tagName).toBe('SELECT')
    expect(within(editableEnvironmentDialog).getByLabelText('network_mode').tagName).toBe('SELECT')
    expect(within(editableEnvironmentDialog).getByLabelText('os').tagName).toBe('SELECT')
    expect(within(editableEnvironmentDialog).getByLabelText('force_build')).toHaveAttribute('type', 'checkbox')
    expect(within(editableEnvironmentDialog).getByLabelText('cpu_enforcement_policy').tagName).toBe('SELECT')
    expect(within(editableEnvironmentDialog).getByLabelText('extra_allowed_hosts')).toBeInTheDocument()
    expect(within(editableEnvironmentDialog).getAllByLabelText('TPU type')[0].tagName).toBe('SELECT')
