...
describe('Environment templates', () => {
  ...
  it('manages OrnnLab-local templates over Harbor config fields', () => {
    ...
    expect(within(editableEnvironmentDialog).getByRole('checkbox', { name: 'network_access' })).toBeChecked()
    expect(within(editableEnvironmentDialog).getByLabelText('allowed_hosts')).toHaveValue('pypi.org, github.com, huggingface.co')
    fireEvent.click(within(editableEnvironmentDialog).getByRole('checkbox', { name: 'network_access' }))
    expect(within(editableEnvironmentDialog).queryByLabelText('allowed_hosts')).not.toBeInTheDocument()
    fireEvent.click(within(editableEnvironmentDialog).getByRole('checkbox', { name: 'network_access' }))
    expect(within(editableEnvironmentDialog).getByLabelText('allowed_hosts')).toHaveValue('*')
