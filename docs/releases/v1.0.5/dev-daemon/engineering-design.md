...
- Related Links: [主题索引](README.md)、[实现设计](implementation-design.md)、[v1.0.5 PRD](../prd.md)、[v1.0.5 技术设计](../technical-design.md)、[v1.0.5 工程计划](../engineering-plan.md)
- Risk Level: Medium
- Plan Type: Standard
...
|---|---|---|---|---|
| `.venv/bin/ornnlab web` / `uv run ornnlab web` fallback | system | Ready | 后端无法启动则 daemon 无法 Running | Phase 1 启动前依赖检查 |
| `npm run dev` | system | Ready | 前端无法启动则 proxy 不可用 | Phase 1 启动前依赖检查 |
| `/api/webui/v1/system/live` | API | Ready | daemon 探活无可信来源 | daemon 只用轻量 live，不引入旧 API |
| `/api/webui/v1/system/health` | API | Ready | System 页无完整体检来源 | 仅用于页面展示，不用于 daemon 存活判断 |
...
- System 页已有 `OrnnLab Service` 体检项，但状态还未绑定应用级 daemon state。
- 文档已明确不做系统级自启动。

## 9. 总体技术设计

实现级设计以 [implementation-design.md](implementation-design.md) 为准。本文只维护阶段计划、验收门禁和完成证据，避免 README、工程计划和实现细节混在一起。
...
flowchart TD
  ...
  SystemAPI["/api/webui/v1/system/health"] --> State
  Daemon --> LiveAPI["/api/webui/v1/system/live"]
...
| API | 行为 |
|---|---|
| `GET /api/webui/v1/system/health` | `OrnnLab Service` 行返回 daemon 状态、前端 URL、日志目录和操作 |
| `GET /api/webui/v1/system/live` | daemon/backend/frontend 专用轻量探活，不执行 Docker、Storage、Hub 或资源体检 |
| `POST /api/webui/v1/system/service/restart` | 请求应用级服务重启，返回 Operation |

首版不新增 `/system/dev-service` 并行接口，直接升级既有 WebUI v1 System API，避免维护两套系统服务契约。停止服务保留在 `ornnlab dev stop` CLI，后续若要进入 WebUI，需要先设计停止当前后端后的用户反馈和恢复路径。
...
- 确认 `GET /system/health` 中 `OrnnLab Service` 的 daemon 状态 DTO。
...
|---|---|---|---|---|---|---|
| 接口冻结 | docs only | docs | 文档治理测试 | none | none | landed |
...
|---|---|---|---|---|
| 范围和接口冻结 | 文档和接口方向已收敛 | complete | no | proceed |
...
| Gate Condition | Verification Evidence | Completion Status | User Approval Required | Proceed Decision |
|---|---|---|---|---|
| 生命周期核心可用 | `npm run test:launcher` | complete | no | proceed |
...
| Gate Condition | Verification Evidence | Completion Status | User Approval Required | Proceed Decision |
|---|---|---|---|---|
| 崩溃恢复可信 | `npm run test:launcher` | complete | no | proceed |

### Phase 3: System API 与前端接入

#### Objective

让 System 页展示真实应用级 dev service 状态，并提供重启操作。
...
后端 System API 从 state 文件和健康检查读取真实状态。前端 System 页通过同一个 `WebUiClient` 访问，不直接读取浏览器状态。首版不开放 WebUI 停止服务按钮，避免停止承载当前页面的后端后无法给用户稳定反馈。

#### Implementation Tasks

- 升级 `/system/health` 中的 `OrnnLab Service` 判别联合成员。
- 让 daemon 启动的后端携带 `ORNNLAB_RESTART_COMMAND`，复用现有 `/system/service/restart`。
- System 页通过分组健康看板中的 `OrnnLab Service` 卡片展示 daemon 状态。
...
| stop action | requested | CLI stop completed | stop failed | error_code | service_id | info/error | user |
...
| Gate Condition | Verification Evidence | Completion Status | User Approval Required | Proceed Decision |
|---|---|---|---|---|
| System 页对齐 daemon | API/frontend/storybook evidence | complete | no | proceed |
...
| Plan Item | Production Code Path | Integration Entry | Test Evidence | Runtime / Log Evidence | Mock / Stub Exposure | Status |
|---|---|---|---|---|---|---|
| full gate | repo scripts | CI/local | `scripts/test-after-change-web.sh` | runtime logs | none | landed |
| rollback | `run_dev.sh` | shell | launcher API-mode foreground regression | run_dev logs | none | landed |
...
| WebUI stop 导致用户无法恢复 | 中 | 用户停止后不知道怎么启动 | 首版不开放 WebUI stop | 用户使用 CLI stop/start |

#### Gate To Next Phase

| Gate Condition | Verification Evidence | Completion Status | User Approval Required | Proceed Decision |
|---|---|---|---|---|
| Stage 7 可关闭 | full gate passed; final subagent review approved | complete | no | proceed |
...
| status health gate | Running 必须同时满足身份可信和 health endpoint 可用 | `lib/dev-daemon.js`、`ornnlab/services/webui_system_service.py` | `ornnlab dev status --json`、System 页 | `tests/node/dev-daemon-hardening.test.js`、`tests/python/test_webui_system_dev_service.py` | status payload / System health row | none | landed |
| stop fail closed | 不可信记录进程仍占用服务端口时 stop 失败，不写 stopped | `lib/dev-daemon.js` | `ornnlab dev stop` | `tests/node/dev-daemon-hardening.test.js` | state.status=error / CLI error | none | landed |
...
| Correctness | API | System health 中的 dev service 状态 | pytest | ApiResponse 和状态映射正确 |
...
- System 页是否在后续版本开放 `停止服务`，以及停止当前后端后的反馈方式。
- daemon 已放在 Node 侧，因为 `ornnlab dev` 发布入口已在 Node。
...
| 2026-07-13 | 0.4 | 最终 subagent 阻断项修复：status/System 必须校验 health endpoint；stop 对不可信但仍服务的记录进程 fail closed；Storybook System 状态对齐后端契约 |
| 2026-07-13 | 1.0 | Stage 7 完成：全量门禁通过，最终 subagent 复审 APPROVED，无 P0/P1/P2 阻断 |
