...
| 文档 | 负责内容 |
|---|---|
| [工程计划](engineering-design.md) | 实施计划、阶段门、完整性矩阵、日志链、测试、回滚与审查标准 |
| [实现设计](implementation-design.md) | 模块边界、状态 schema、进程身份、生命周期、日志脱敏、CLI/API 契约、测试门禁 |
...
System 页 `OrnnLab Service` 卡片应对齐 CLI 能力：

- 状态：`Running`、`Stopped`、`Starting`、`Degraded`、`Restarting`、`Error`
- 服务地址：当前前端 URL
- 日志路径：dev service 日志目录
- 操作：检查更新、重启
- 操作：`检查更新`、`重启`
