...
- 卡片只展示本地化的错误摘要；原始错误保留在 API 和服务日志中，路径是弱化信息。
- 状态码由后端按事实计算，展示文案和颜色由前端 i18n 映射。
- Docker 只有 daemon 可连接时才允许清理 Docker 缓存。
- Docker 探测只依赖当前 Context 和标准 `docker version` 输出，返回 Client/Server 版本；不识别具体 Runtime，也不维护 Runtime 适配器。
- 用户可保存一个本机启动命令。后端只接受单一可执行文件及参数，以 `shell=false` 异步执行；完成后轮询当前 Docker Context。该能力不推断命令、不安装 Runtime，也不提供关闭操作。
...
Storybook 状态矩阵覆盖：正常看板、Docker 未运行、Docker 快捷启动、服务降级、GPU 未检测、存储不足、中文、空状态和破坏性操作确认。全局 Storybook theme toolbar 继续覆盖深色主题。故事使用 mock DTO，不连接真实系统服务。
