...
| API contract client | 部分完成 | 已有契约类型、HTTP/mock client、runtime mode、Jobs/Datasets 的列表/详情/events/trials/tasks hooks 与 DTO/ViewModel；Agents、Environments、Leaderboard、System 和 Operation 状态流仍待迁移；不建设 legacy adapter |
| 后端 API 破坏性升级 | 未开始 | 后端旧 `/api/experiments`、`/api/runs`、`/api/benchmarks` 等接口需要直接升级为 `/api/webui/v1` 产品契约，不维护新旧两套 |
| 联调门禁 | Stage 1 门禁已通过 | 当前已通过 typecheck、unit、lint、build、Storybook smoke、e2e；进入联调前仍需补 API 层和真实 contract smoke |
