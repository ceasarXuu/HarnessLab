...
| 领域类型治理 | Stage 2 已完成 | 生产 UI 类型来自 `domain` / `api`；`app`、`screens`、`ui/components` 不再导入 mock fixture；OpenCode 整改复审已通过 |
| API contract client | Stage 2 已完成 | 六类资源、详情附属资源与 Header Hub 状态均经 DTO、HTTP/mock client、hook、ViewModel 读取；全部可见写操作经 `Operation`，不建设 legacy adapter 或 API 成功 fallback |
| 后端 API 破坏性升级 | 未开始 | 后端旧 `/api/experiments`、`/api/runs`、`/api/benchmarks` 等接口需要直接升级为 `/api/webui/v1` 产品契约，不维护新旧两套 |
| 联调门禁 | Stage 2 已完成 | 已通过 typecheck、48 个 unit、lint、build、Storybook smoke、10 个 desktop/mobile e2e；Stage 3 还需真实 contract smoke |
