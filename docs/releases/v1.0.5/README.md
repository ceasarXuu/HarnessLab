...
- Agent/模型调优者：需要比较不同 agent、模型、skills、MCP 配置和 Environment profile。
...
- Harbor JobConfig 可视化配置：
  ...
  - Environment profile 选择；Docker 资源、mounts、delete/force-build 等细节收敛到 Environment 一级页；
...
4. 用户进入 Jobs/New Job，选择 dataset、task filter、attempt、并发、agent 和 Environment profile。
