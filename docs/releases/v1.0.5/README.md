...
- Harbor JobConfig 可视化配置：
  ...
  - dataset、version、task filter、include/exclude、n_tasks；
...
3. 用户进入 Jobs/New Job，选择 dataset、task filter、attempt、并发、agent 和环境参数。
4. Web 展示即将生成的 Harbor JobConfig 摘要，用户确认后启动。
5. Web 实时展示 job 状态、job log、events 和 job 下属 trial 进度。
...
- Visual baseline:
  ...
  - 首页优先呈现 Harbor-style Dataset catalog 和 selected dataset task detail，不再使用重卡片式运营仪表盘作为第一视觉。
