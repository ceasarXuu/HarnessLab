...
| 页面/组件 | 必须覆盖 |
|---|---|
| App Shell | dark/light、zh/en、Hub connected/disconnected、API unavailable |
| Jobs | loaded、empty、loading、failed、drawer loaded、event stream disconnected、operation running |
| New Job | default valid、dataset no tasks、verifier skip、task search bulk select、run validation failed |
| Datasets | registry not downloaded、downloading、downloaded、local import、delete confirm、split empty、task row expanded |
| Agents | built-in immutable、custom deletable、missing secret、skills empty/path configured、MCP variants |
| Environment | built-in/custom、network off/allowlist、GPU/TPU、advanced collapsed/expanded、healthcheck enabled/disabled |
| System | healthy/degraded、update available、cache operation running、destructive confirm、toast countdown |
