...
- `GET /model-pricing/preview?modelName=` 复用 `model_pricing.catalog_pricing` 返回当前安装的 LiteLLM 目录价格。Agent 编辑器在 `reported` 与 `litellm` 模式均展示该结果：前者是参考单价且仍采用 Harness 上报总成本，后者与 Job 创建时的快照解析同源。目录未命中时 API 返回显式错误，前端显示价格不可用，不静默填零。
