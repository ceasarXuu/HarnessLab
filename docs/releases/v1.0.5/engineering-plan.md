...
- Agent 模型设置提供独立价格来源选择；自定义模式可填写输入缓存未命中、输入缓存命中和输出单价，LiteLLM 与 Harness 模式展示只读说明，并由 Storybook 覆盖交互状态。
