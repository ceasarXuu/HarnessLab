...
| S3-09 | 前端约束一致 | Agent 保存可选模型集合，New Job 单选具体模型；built-in Agent 配置可编辑但不可删除或更换 Harness；mock 与 API 约束一致 | Done |
...
- Job 创建接受 OrnnLab Agent 配置；系统预置 Agent 首次使用时自动物化。Agent 以 Harbor `AgentConfig` 校验，Environment 以 `EnvironmentConfig` 校验。
...
- built-in Agent 表示引用 Harbor 内置 Harness 的系统预置配置；名称及能力子集内的可选模型集合、环境变量、Skills、MCP、kwargs 和超时可编辑，Harness 身份固定且资源不可删除。
