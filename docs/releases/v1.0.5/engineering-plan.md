...
| ID | 验收项 | 完成证据 | 状态 |
|---|---|---|---|
| S4-01 | 联调启动器 | `run_dev.sh` 同时启动后端与 5173 前端，默认 `VITE_ORNNLAB_DATA_MODE=api`，`ORNNLAB_API_TARGET` 可覆盖代理目标；直接 `npm run dev` 仍默认 mock | In progress |
| S4-02 | API 读取全覆盖 | Jobs、Datasets、Agents、Environments、Leaderboard、System 与 Hub 页面均通过 5173 proxy 读取真实后端，并在 API 错误时显示错误而非 mock 数据 | Pending |
| S4-03 | 资源写操作 | custom Agent、custom Environment 与 Job leaderboard 开关在真实 API 中完成受控创建、更新、删除/恢复及 Operation 刷新验证 | Pending |
| S4-04 | Dataset 异步操作 | 使用 Harbor registry 的最小 Dataset 验证 download、poll、cancel 或最终完成、删除本地数据的真实状态变化 | Pending |
| S4-05 | 真实 Harbor Job | 使用已下载的最小 Dataset、Docker 环境和可执行 custom Agent 完成一次实际 Harbor Job；记录 Job、Trial、事件与产物读回 | Pending |
| S4-06 | Docker 与 Hub 可观察性 | System 显示实际 Docker 状态；Hub 无凭证时显示 disconnected，有凭证时显示真实连接状态，不伪造成功 | In progress |
| S4-07 | 前端 API 可视验收 | Codex Web Preview 在 API 模式验收六个一级页面、New Job 和 Operation 刷新，确认无 mock 回退与控制台错误 | Pending |
