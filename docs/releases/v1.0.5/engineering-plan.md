...
| 10 | 模型价格配置 | Done | 每模型价格来源、自定义缓存价格、Job 快照与成本重算已实现；全量门禁通过 |
| 11 | Job 完整删除 | Done | 终态约束、全记录事务清理、独占产物安全删除、确认交互和全量门禁均已完成 |
| 12 | Docker 所有权与回收 | Done | 实例隔离标签、Harbor Compose 全服务注入、终态/启动回收、真实 Harbor 冒烟和全量门禁均已通过 |
...
| S3-05 | Job 真实映射 | New Job payload 映射 Harbor `JobConfig` 真实字段；Agent 配置与 Environment 模板在后端展开；Job 所选模型写入 `AgentConfig.model_name` | Done |
...
| S3-09 | 前端约束一致 | Harness 模板与 Agent profile 分离；Agent 保存可选模型集合，New Job 单选具体模型；mock 与 API 约束一致 | Done |
...
| S4-03 | 资源写操作 | Agent、custom Environment 与 Job leaderboard 开关已通过真实 API 创建、更新、删除或恢复，并读取最终资源状态 | Done |
...
- migration `006_unify_agent_configs.sql` 已将 Agent 配置收敛到 `agents.config_json`；旧 `webui_agent_configs`、AgentProfile 文件、ProfileCompiler 和 generated-agent 运行路径均已移除。
- Job 创建只接受已保存的 OrnnLab Agent profile；Harbor Harness 通过 `/harnesses` 作为创建模板提供，不自动物化。Agent 以 Harbor `AgentConfig` 校验，Environment 以 `EnvironmentConfig` 校验。
- Job 输出只读取 Harbor 实际 `TrialResult` 字段；不存在的进度、日志或验证器字段返回空值，不编造。
- Agent 可为每个模型分别选择 Harness 上报价格、LiteLLM 定价表或自定义价格；Job 创建时固化价格快照，并统一按输入缓存未命中、输入缓存命中和输出 token 重算 Job、Trial 与排行榜成本。
- Job 列表 DTO 将 Harbor 终态拆为 `completed/passed/notPassed/errored`；运行时长对 running Job 计算到当前时间，对终态 Job 固定到完成时间。运行态在数据库 `result_path` 尚未写入时安全读取当前 Harbor Job 的原生结果，避免全量 Dataset 的 `n_tasks=NULL` 被长期显示为 0。
- Job 删除只接受终态 Job，在同一数据库事务内清理 Run、队列、配置、事件、Operation 和无其他 Run 的 Experiment；排行榜由剩余 Run 实时派生。删除器仅移除可证明归属当前 Job 的产物根目录，拒绝越界、符号链接或归属不明路径，并记录删除前后结构化日志。
- Docker Job 通过 OrnnLab 自定义 Harbor Environment 给 main 与 sidecar 注入实例/run
  所有权标签，并给 Compose 网络与卷写入同一协议；Harbor 返回后和应用启动恢复时按标签幂等回收容器、空 Compose 网络和卷，
...
- New Job 已去除 `split`、custom verifier、`env_file`、输出 tab 与虚构环境字段；提交 Harbor 支持的 Job 级字段，并从 Agent 的可选模型集合中提交唯一 `modelName`。
- Agent 目录只展示用户保存的配置；Harbor 内置的 30 种 AgentName 仅进入 Harness 模板目录。新建 Agent 选择 Harness 后配置模型集合、环境变量、Skills、MCP、kwargs 和超时；保存后才进入 Agent 目录。
...
- Jobs 表格移除状态列，展示任务总数、通过/未通过和异常失败；运行中名称使用 loading 动画，已有每秒 Job 轮询同时驱动计数与秒表。
...
```text
frontend npm test                         31 files, 110 tests passed
frontend npm run lint                     passed
frontend npm run typecheck                passed
frontend npm run build                    passed
frontend npm run storybook:test           passed
frontend npm run storybook:build          passed
.venv/bin/python -m pytest tests/python -q 161 passed, 3 skipped
...
2026-07-21 的 Stage 9 真实 API 验证：`run-371699db5dee` 返回
`total=10, completed=8, passed=4, notPassed=4, errored=2`，满足终态关系
`10 = 8 + 2`；后端直连与 5173 代理结果一致。`scripts/test-after-change-web.sh`
完整通过，包含 Ruff、Pyright、Python 161 passed / 3 skipped、前端 110 tests、
lint、typecheck、生产构建、Storybook smoke/static build 和 launcher 回归。

2026-07-21 的 Stage 10 全量门禁通过：Ruff、Pyright（0 error / 0 warning）、
Python 168 passed / 3 skipped、前端 32 files / 114 tests、lint、typecheck、
生产构建、bundle size、Storybook smoke/static build、launcher 27/27 和
`git diff --check` 均为绿色。测试覆盖旧 Agent 默认迁移、三种价格来源、自定义
缓存价格计算、LiteLLM 快照和价格预览、缺少缓存用量时拒绝误算，以及前端价格编辑交互。

2026-07-22 的 Stage 11 全量门禁通过：Ruff、Pyright（0 error / 0 warning）、
Python 171 passed / 3 skipped、前端 32 files / 117 tests、lint、typecheck、
生产构建（JS 404.32 KiB / CSS 39.95 KiB）、bundle size、Storybook smoke/static build
和 launcher 27/27。回归覆盖终态完整删除、活动 Job 拒绝、归属不明产物拒绝、
共享目录保留、排行榜同步消失，以及前端终态确认与运行中入口隐藏。

2026-07-22 的 running Job 计数与事件脱敏修复全量门禁通过：Ruff、
Pyright（0 error / 0 warning）、Python 176 passed / 3 skipped、前端
32 files / 117 tests、lint、typecheck、生产构建、bundle size、Storybook
...
2026-07-23 的 Docker 所有权与回收全量门禁通过：Python 187 passed / 4 skipped、
Ruff、Pyright（0 error / 0 warning）、前端 32 files / 117 tests、lint、typecheck、
生产构建、bundle size、Storybook smoke/static build 和 launcher 27/27。随机真实
Compose 双服务冒烟证明 main、sidecar 与网络均带实例/run 标签，回收 2 个容器和 1 个网络后
...
- Docker 容器标签在创建后不可修改；所有权必须在 Compose `up` 前作为最终覆盖层注入。
