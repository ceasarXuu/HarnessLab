...
| S3-05 | Job 真实映射 | New Job payload 映射 Harbor `JobConfig` 真实字段；Agent 配置与 Environment 模板在后端展开 | Done |
...
| S3-09 | 前端约束一致 | New Job 可选择全部 Agent 配置；built-in Agent 配置可编辑但不可删除或更换 Harness；mock 与 API 约束一致 | Done |
...
- built-in Agent 表示引用 Harbor 内置 Harness 的系统预置配置；名称及能力子集内的模型、凭证、Skills、MCP、kwargs 和超时可编辑，Harness 身份固定且资源不可删除。
