...
| Verifier | override/max timeout、env、import_path、kwargs、disable | New Job 验证器 tab 以“验证方式”下拉作为主入口：默认使用 Dataset 验证器；自定义模式展开 import_path、env、kwargs、max timeout；跳过验证模式映射 disable | Partial | 后端接入时将 UI mode 映射为 Harbor `VerifierConfig` / disable 语义，并对跳过验证的 leaderboard 影响做限制。 |
