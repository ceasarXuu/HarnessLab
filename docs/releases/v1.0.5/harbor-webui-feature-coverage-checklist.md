...
| Environment | 搜索、新建 custom 模板、复制模板、编辑/删除 custom 模板、点击行打开 Environment drawer | 主要为 seed 数据；新建/复制为二级页面，详情和编辑在 drawer；CRUD 语义是 OrnnLab-local 模板管理，不是 Harbor 原生命令。 |
