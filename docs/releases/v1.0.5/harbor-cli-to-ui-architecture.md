...
| 编辑 dataset manifest | `harbor add/remove/sync` | 向 dataset.toml 添加、移除、同步 digest | Dataset detail 的 Manifest 工具区；不放在顶部快捷操作中 | P1 |
...
- Dataset detail：点击 dataset 列表行后从右侧 drawer 滑出，展示 task 列表、版本、manifest 路径、下载/拉取更新/发布入口；顶部快捷区只保留面向使用者的操作。
- Task 是 Dataset 的子概念，不作为 v1.0.5 一级页面。所有 task 浏览、搜索、描述、下载、
  check/debug、start-env、run single task 都从 Dataset detail 进入。
- Dataset editor：替代 `add/remove/sync`，作为 Manifest 工具区展示 Harbor 当前支持的 manifest 维护能力，不作为顶部快捷按钮。
