...
`harbor run` 参数面最大，但 v1.0.5 的 New Job 页面不采用步骤条。基础字段数量不多，应直接铺开；
高级字段按稳定子域分组折叠，用户可以在同一个页面内完成配置、预览和运行。
...
- Job list：本地 jobs。
- New job：JobConfig 单页铺开表单，基础配置直接可见，高级配置按区域折叠。
...
3. JobConfig 单页表单：Source、Filter、Agent、Environment、Runtime、Preview。
4. Run / Resume。
