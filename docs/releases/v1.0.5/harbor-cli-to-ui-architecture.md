...
| 下载 job | `harbor job download` | 拉取 Hub 上的 job 和 trials | Jobs 页 Import from Hub 表单：job id/url、download target、conflict policy | P2 |
...
| 编辑 dataset manifest | `harbor add/remove/sync` | 向 dataset.toml 添加、移除、同步 digest | 不进入详情抽屉；后续作为 Dataset Editor + manifest diff | P2 |
...
页面按使用频率和功能领域拆成子 tab：基础参数默认可见，task 过滤与数量进入 Tasks，低频参数进入验证器、运行策略、
运行策略等领域 tab。用户仍然在同一个页面内完成配置、预览和运行；环境细节配置不进入 New Job，而是由独立 Environment 一级页管理。输出、上传、metric 与 plugins 不作为 New Job 创建前决策项展示。
...
| 运行后输出 | `--upload`, `--public/--private`, `--share-org`、artifacts、plugins、metrics | 不进入 New Job；上传放到 Job detail，artifacts 作为运行后路径展示，metric 归入 Leaderboard，plugins 待真实插件列表后再设计 |
...
- Job detail：点击 job 列表行后从右侧 drawer 滑出，展示 events、logs、trials、artifacts、config、summary、upload/share、resume；Hub job 下载不放在本地 Job drawer 内，归入 Jobs 页 Import from Hub。
...
- Dataset detail：点击 dataset 列表行后从右侧 drawer 滑出，默认展示 task 数、source、本地路径、大小、registry 与 task 列表；task 列表支持 split 筛选与搜索；下载/拉取更新/删除本地数据按状态展示。digest/ref/manifest 命令等底层字段不默认展示。
- Task 是 Dataset 的子概念，不作为 v1.0.5 一级页面。所有 task 浏览、搜索、描述、下载、
  check/debug、start-env、run single task 都从 Dataset detail 进入。
- Dataset editor：替代 `add/remove/sync`，后续以独立编辑流程和 manifest diff 展示，不放在 Dataset detail 抽屉内。
...
- 一级页面：管理可复用运行环境配置。
- 每个 Environment 显示名称、backend、类型、镜像、资源策略、状态和来源。
- Environment detail：点击环境列表行后从右侧 drawer 滑出，展示 backend、image、resources、mounts、env、allowed hosts、force build、delete after run。
- New Job 的 Environment 字段必须从 Environment 页的可用 profile 中选择，不再让用户临时配置资源、mount 或 compose overlay。
...
3. JobConfig 单页多 tab 表单：基础、Tasks、Verifier、Runtime、Preview；Environment 作为 profile 下拉选择。
4. Run / Resume。
