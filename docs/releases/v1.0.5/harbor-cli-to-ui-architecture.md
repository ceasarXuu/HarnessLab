...
concurrency、attempts、debug、env_file、计入排行榜。task split 和 Task 白名单进入 Tasks tab，默认全选；用户通过列表开关形成 Harbor `task_names` 白名单。搜索过滤只改变当前可见 task 集合，批量开启/关闭仅作用于当前过滤结果。
