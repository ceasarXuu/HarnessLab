...
输出等领域 tab。用户仍然在同一个页面内完成配置、预览和运行；环境细节配置不进入 New Job，而是由独立 Environment 一级页管理。
...
3. JobConfig 单页多 tab 表单：基础、Tasks、Verifier、Runtime、输出、Preview；Environment 作为 profile 下拉选择。
4. Run / Resume。
