...
已下载或本地导入的 Dataset 应从每个 Task 的 `task.toml` 和 `environment/` 目录解析运行环境摘要，在 Task 展开区展示操作系统、预构建镜像、Dockerfile/Docker Compose 定义、构建超时、网络模式、资源要求和工作目录。远程但未下载的 Registry Dataset 只展示 Registry 已提供的 Task 名称，并提示下载后可查看运行环境。v1.0.5 不查询镜像 manifest、不判断本机兼容性、不构建替代镜像，也不因解析结果阻止 Job。
...
- 不根据 Dataset Task 的运行环境摘要推断本机兼容性，也不管理重建镜像。
