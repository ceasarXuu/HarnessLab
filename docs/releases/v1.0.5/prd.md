...
- Environment 承载可复用运行环境配置管理；New Job 只选择已配置好的 Environment。
...
- Environment：展示可复用环境配置，支持搜索、新建 custom environment、查看配置详情、删除 custom environment。
...
用户点击“新建 Job”进入二级页面 `/jobs/new`。页面按配置领域分组：基础、Tasks、Verifier/运行时、输出。基础区保留高频 Agent/Harness、Environment 选择和“计入排行榜”开关。Environment 是已配置好的可复用环境 profile，不在 New Job 中展开资源、镜像、mount、环境变量等细节。Tasks 展示 split 与 Task 白名单列表，默认全选；用户可搜索过滤、单项开关、全部开启或全部关闭。存在搜索过滤时，全部开启/全部关闭只作用于当前搜索结果；无搜索过滤时作用于完整 task 列表。用户确认配置后运行 Job，回到 Jobs 列表并打开新 Job 详情。
...
用户进入 Environment 页面后可以搜索可复用环境配置，也可以新建 custom Environment。列表展示 Environment Name、Backend、类型、镜像、资源策略、状态和操作。built-in Environment 不可删除；custom Environment 可删除，删除前二次确认。

点击 Environment 行打开右侧详情抽屉，查看 backend、image、resources、mounts、env、allowed hosts、force build、delete after run 和来源。New Job 仅通过下拉选择这些 Environment profile。
...
- New Job 不展示 Environment 配置子 tab，只从 Environment profile 下拉选择。
...
| 一级导航 | Jobs、Datasets、Agents、Environment、Leaderboard、System | Environment 作为可复用配置资源独立管理，避免 New Job 暴露环境细节 | 前序讨论 |
