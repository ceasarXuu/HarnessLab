...
- Agent 的“环境变量”不是凭证库。新增变量时优先从当前 Harness 的 Harbor `ENV_VARS` 清单中选择，也允许切换为自定义变量名；已选择变量不得重复。每项可选择继承 OrnnLab 进程中的同名变量，或保存固定字符串；API 用 `null` 表示继承。固定值会保存在本地 Agent profile 中，因此敏感凭证应优先使用继承方式，直至独立 Secret 管理能力落地。
