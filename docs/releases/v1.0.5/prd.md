...
用户点击“新建 Job”进入二级页面 `/jobs/new`。页面按配置领域分组：基础、Tasks、环境资源、Verifier/运行时、输出。基础区保留高频 Agent/Harness 选择和“计入排行榜”开关。Tasks 默认全量运行，只展示 split 与运行范围；用户选择自定义范围后，再展示 include/exclude 和数量上限。用户确认配置后运行 Job，回到 Jobs 列表并打开新 Job 详情。
