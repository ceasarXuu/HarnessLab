...
产品定位是“基于 Harbor 的本地实验控制台”。当前版本先完成 Harbor WebUI 产品形态：围绕 Job、Dataset、Agent、Leaderboard、System 五个一级页面，提供创建、运行、查看、管理、诊断等核心流程。
...
用户点击“新建 Job”进入二级页面 `/jobs/new`。页面按配置领域分组：基础、Tasks、环境资源、Verifier/运行时、输出。基础区保留高频 Agent/Harness 选择和“计入排行榜”开关。Tasks 展示 split 与 Task 白名单列表，默认全选；用户可搜索过滤、单项开关、全部开启或全部关闭。存在搜索过滤时，全部开启/全部关闭只作用于当前搜索结果；无搜索过滤时作用于完整 task 列表。用户确认配置后运行 Job，回到 Jobs 列表并打开新 Job 详情。
...
- 一级导航只有 Jobs、Datasets、Agents、Leaderboard、System。
- Jobs 是默认第一个 tab。
...
| 一级导航 | Jobs、Datasets、Agents、Leaderboard、System | 与 Harbor 资源层级一致，避免 Tasks 独立成一级页面 | 前序讨论 |
