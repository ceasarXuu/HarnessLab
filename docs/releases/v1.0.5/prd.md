...
用户点击“新建 Job”进入二级页面 `/jobs/new`。页面按配置领域分组：基础、Tasks、Verifier/运行时、输出。基础区保留高频 Agent/Harness、Environment 选择和“计入排行榜”开关。Environment 是 OrnnLab-local 环境模板，不在 New Job 中展开资源、镜像、mount、环境变量等细节。Tasks 展示 split 与 Task 白名单列表，默认全选；用户可搜索过滤、单项开关、全部开启或全部关闭。存在搜索过滤时，全部开启/全部关闭只作用于当前搜索结果；无搜索过滤时作用于完整 task 列表。用户确认配置后运行 Job，回到 Jobs 列表并打开新 Job 详情。
...
点击 Environment 行打开右侧详情抽屉，直接编辑 Harbor 真实支持的环境字段：`type` / `import_path`、`network_mode` / `allowed_hosts`、`docker_image`、`os`、CPU/Memory/Storage/GPU/TPU、`skills_dir`、`healthcheck`、`workdir`、`mounts`、`env`、`kwargs`、`force_build`、`delete`、资源覆盖项和 `extra_docker_compose`。custom 模板允许删除，删除前二次确认。新建 Environment 和复制模板进入二级页面，保存后回到列表并打开对应详情抽屉。New Job 仅通过下拉选择这些模板。

Environment 字段交互按 Harbor 字段类型分配控件：`type`、`os`、`network_mode`、CPU/Memory enforcement policy 使用下拉列表；`force_build`、`delete` 使用 switch；CPU/Memory/Storage/GPU 数量使用数字输入；`allowed_hosts`、`extra_allowed_hosts`、`gpu_types` 使用逗号分隔的 tag 输入；`env`、`kwargs` 使用 Key-Value 多行输入；`mounts`、`healthcheck` 使用 JSON 多行输入；`extra_docker_compose` 和目录类字段使用路径输入。`allowed_hosts` 只表示 task `[environment]` 基线，运行时增量 host 使用 `extra_allowed_hosts`。
