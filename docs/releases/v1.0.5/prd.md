...
点击 dataset 行打开右侧详情抽屉，查看 task 数、来源、splits、本地路径、大小、registry 和 task 列表。抽屉默认不展示 digest、ref、include/exclude、overwrite、manifest 命令等底层细节；Manifest 编辑后续作为独立 Dataset Editor 处理。详情操作按下载状态展示：未下载显示下载，下载中显示进度与取消，已下载 registry dataset 显示拉取更新和删除本地数据，本地导入 dataset 不显示拉取更新。Task 作为 dataset 子资源展示，支持搜索、展开描述，并可从 task 创建单 task Job。
