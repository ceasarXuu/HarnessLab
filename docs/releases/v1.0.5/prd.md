...
- 当前 UI 优化阶段不接入真实后端；进入联调时必须通过 WebUI contract client / adapter 接入，不允许页面直接消费旧后端路由。
- 不做独立的 Tasks 一级页面。
...
- 进入联调前，前端必须通过 `frontend/src/api/` contract client 或 adapter 访问后端，不允许 screen/component 直接消费旧路由。
...
- 后端是否直接新增 `/api/webui/v1` facade，还是先由前端 adapter 兼容旧 `/api/experiments`、`/api/runs` 等路由？
