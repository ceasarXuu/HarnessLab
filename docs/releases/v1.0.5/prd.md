...
每个 Agent 模型独立选择成本来源：Harness 上报、LiteLLM 目录或自定义价格。自定义价格按每百万 Token 分别填写输入缓存未命中、输入缓存命中和输出单价。创建 Job 时必须把本次模型的已解析价格保存为不可变快照；后续 Agent 编辑或 LiteLLM 目录更新不能改变历史 Job。Jobs、Trial 和 Leaderboard 使用同一快照与 Harbor Token 统计计算成本；原始 Harness 上报金额保留在 Harbor 结果中，不被 OrnnLab 改写。缓存命中/未命中价格不同时，如果 Harbor 没有提供缓存 Token，用 `-` 表示无法估算，不得默认为零命中。
