...
- Status reason: 已有一轮较完整的前端功能讨论，前端 mock 产品化 Stage 1 已完成；进入联调前还需要完成 API contract client、domain 类型迁移、后端旧 API 破坏性升级和真实 contract smoke。
...
- 联调准备：前后端联调前必须满足 `docs/releases/v1.0.5/frontend-backend-integration-readiness.md` 的设计基线。

### Out Of Scope

- 当前 UI 优化阶段不接入真实后端；进入联调时必须通过 WebUI contract client 接入，不允许页面直接消费旧后端路由，也不维护前端 legacy adapter。
- 不做独立的 Tasks 一级页面。
...
点击 dataset 行打开右侧详情抽屉，查看 task 数、来源、本地路径、大小、registry 和 task 列表。抽屉默认不展示 digest、ref、include/exclude、overwrite、manifest 命令等底层字段；Manifest 编辑后续作为独立 Dataset Editor 处理。详情操作按下载状态展示：未下载显示下载，下载中显示进度与取消，已下载 registry dataset 显示拉取更新和删除本地数据，本地导入 dataset 不显示拉取更新。Task 作为 dataset 子资源展示，支持 split 筛选、搜索、展开描述，并可从 task 创建单 task Job。
...
Environment 字段交互按 Harbor 字段类型分配控件：`type`、`os`、`network_mode`、CPU/Memory enforcement policy 使用下拉列表；`force_build`、`delete`、`healthcheck` 启用状态使用 switch；CPU/Memory/Storage/GPU 数量使用数字输入；`tpu` 使用 TPU type 下拉 + Topology X/Y/Z 数字输入，并由 UI 组合成 Harbor 的 `TYPE=TOPOLOGY` 值；`allowed_hosts`、`extra_allowed_hosts`、`gpu_types` 使用可添加/删除的列表或 tag 输入；`env`、`kwargs` 使用 Key-Value 多行输入；`mounts` 使用结构化路径/JSON 输入；`healthcheck` 放在高级 tab，以 command、interval、timeout、start period、start interval、retries 的结构化字段维护；`extra_docker_compose` 和目录类字段使用路径输入。`allowed_hosts` 只表示 task `[environment]` 基线，运行时增量 host 使用 `extra_allowed_hosts`。
...
- 进入联调前，e2e、unit test 和 Storybook play 必须全绿且断言口径一致。
- 进入联调前，生产 UI 类型不得继续从 `frontend/src/mocks/` 导出；mock 只作为 Storybook 和测试 fixture。
- 进入联调前，前端必须通过 `frontend/src/api/` contract client 访问后端，不允许 screen/component 直接消费旧路由。
...
- 后端旧 `/api/experiments`、`/api/runs`、`/api/benchmarks` 等路由的物理保留或移除策略如何安排？产品契约必须直接升级到 `/api/webui/v1`，不做新旧并行兼容。
- Storybook 是否引入 MSW，还是先用纯 fixture/data hook mock 作为接口状态评审面？
...
| 联调准备 | Stage 1 前端 mock 产品化完成后，先补 WebUI contract client、domain 类型和真实 contract smoke，再把旧后端 API 破坏性升级为新契约 | 防止页面直接绑定旧 API 或 mock 字段，避免维护新旧两套接口 | 2026-07-10 用户确认 |
