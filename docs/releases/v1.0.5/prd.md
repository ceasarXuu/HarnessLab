...
本地 Docker Job 默认自动继承 OrnnLab 进程中的标准 `HTTP_PROXY`、
`HTTPS_PROXY`、`ALL_PROXY` 与 `NO_PROXY` 大小写变量。非回环代理直接继承；
本地 rootful Docker 遇到 `localhost`、`127.0.0.0/8` 或 `::1` 代理时，
OrnnLab 在执行期建立只绑定 Docker bridge 的临时 TCP relay，再把派生地址作为
Agent 运行默认值注入。Agent 或 Environment Profile 的显式代理配置优先，
不得被自动发现覆盖。用户可通过 `ORNNLAB_DOCKER_PROXY_MODE=off` 关闭自动继承。
无法安全转换的代理必须在 Harbor 启动前明确失败，不得静默直连或记录代理凭据。
