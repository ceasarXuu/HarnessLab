...
- Agent 的“环境变量”不是凭证库。新增变量时优先从当前 Harness 的 Harbor `ENV_VARS` 清单中选择，也允许切换为自定义变量名；已选择变量不得重复。取值方式默认“自定义”，展示值输入框并保存用户填写的字符串；切换为“继承系统变量”时不展示值输入框，只在运行时读取 OrnnLab 进程中的同名变量，API 用 `null` 表示继承。自定义值会保存在本地 Agent profile 中，因此敏感凭证应优先使用继承方式，直至独立 Secret 管理能力落地。
...
Agent 是 OrnnLab 保存的可复用运行配置，组合 Harness、可选模型集合、环境变量、Skills、MCP、专属参数和超时。`built-in` 仅表示配置引用 Harbor 内置 Harness，不表示配置只读；这类系统预置配置可编辑、可直接用于 Job，但不能删除，且不能更换 Harness。自定义 Harness 配置可编辑和删除。New Job 不暴露 Agent 的其他内部字段，但必须从所选 Agent 的可选模型集合中单选本次运行模型。
