...
- Environment：管理 OrnnLab-local Environment 模板，支持搜索、新建、复制、编辑、删除 custom 模板、查看配置详情；built-in 模板不可直接编辑或删除，只能复制为 custom 模板后修改。新建和复制使用二级页面，详情使用右侧抽屉，custom 模板可在抽屉中直接编辑。
...
点击 Environment 行打开右侧详情抽屉，查看 Harbor 真实支持的环境字段：`type` / `import_path`、`network_mode` / `allowed_hosts`、`docker_image`、`os`、CPU/Memory/Storage/GPU/TPU、`skills_dir`、`healthcheck`、`workdir`、`mounts`、`env`、`kwargs`、`force_build`、`delete`、资源覆盖项和 `extra_docker_compose`。built-in 模板只允许复制；custom 模板允许在抽屉中直接编辑和删除，删除前二次确认。新建 Environment 和复制模板进入二级页面，保存后回到列表并打开对应详情抽屉。New Job 仅通过下拉选择这些模板。
