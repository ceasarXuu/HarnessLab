...
- Environment 承载 Harbor 环境参数预设的查看与选择；New Job 只从这些预设中选择，不在新建 Job 流程里展开底层环境细节。新建/删除 Environment profile 不是当前 Harbor 原生能力，需等 OrnnLab 后端定义本地 profile API 后再进入范围。
...
- Environment：展示 Harbor `EnvironmentConfig` / `harbor job start` 支持的环境参数预设，支持搜索和查看配置详情；不提供 Harbor 没有的一等资源新建/删除动作。
...
- 不做 Environment profile 的新建/删除，除非后续明确由 OrnnLab 本地 profile API 支撑。
...
用户点击“新建 Job”进入二级页面 `/jobs/new`。页面按配置领域分组：基础、Tasks、Verifier/运行时、输出。基础区保留高频 Agent/Harness、Environment 选择和“计入排行榜”开关。Environment 是 Harbor 环境参数预设，不在 New Job 中展开资源、镜像、mount、环境变量等细节。Tasks 展示 split 与 Task 白名单列表，默认全选；用户可搜索过滤、单项开关、全部开启或全部关闭。存在搜索过滤时，全部开启/全部关闭只作用于当前搜索结果；无搜索过滤时作用于完整 task 列表。用户确认配置后运行 Job，回到 Jobs 列表并打开新 Job 详情。
...
用户进入 Environment 页面后可以搜索 Harbor 环境参数预设。列表展示 Environment Name、`type`、`docker_image`、`network_mode`、CPU/Memory 策略和运行时覆盖项。

点击 Environment 行打开右侧详情抽屉，查看 Harbor 真实支持的环境字段：`type` / `import_path`、`network_mode` / `allowed_hosts`、`docker_image`、`os`、CPU/Memory/Storage/GPU/TPU、`skills_dir`、`healthcheck`、`workdir`、`mounts`、`env`、`kwargs`、`force_build`、`delete`、资源覆盖项和 `extra_docker_compose`。New Job 仅通过下拉选择这些预设。
...
- New Job 不展示 Environment 配置子 tab，只从 Environment 参数预设下拉选择。
...
| 一级导航 | Jobs、Datasets、Agents、Environment、Leaderboard、System | Environment 作为 Harbor 环境参数预设查看页，避免 New Job 暴露环境细节；新建/删除 profile 暂不承诺 | 2026-06-30 Harbor CLI/模型核验 |
