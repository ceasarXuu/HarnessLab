...
built-in Harness 模板只读，详情展示该 Harness 可生成的完整配置子集，并可基于模板新建 Agent。Agent 是 OrnnLab 保存的可复用运行配置，组合 Harness 与模型、凭证、Skills、MCP、专属参数和超时；custom Agent 可编辑与删除。New Job 只选择已保存的 Agent，不暴露 Agent 内部字段。
