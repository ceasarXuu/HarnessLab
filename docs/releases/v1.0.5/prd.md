...
所有新建/保存类表单使用统一合法性反馈：操作按钮仅在无写权限或请求执行中禁用，不因字段缺失而静默禁用；用户提交不合法表单后，页面顶部显示可点击的错误摘要，具体字段同时显示错误状态和文案，点击摘要可定位字段。首次提交后错误随编辑实时更新。API 或异步 Operation 拒绝请求时，错误显示在同一区域，不在页面底部重复展示。该规范由共享组件和 Storybook 状态约束管理。
