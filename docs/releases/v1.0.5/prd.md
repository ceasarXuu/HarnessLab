...
Jobs 是默认页面。列表展示名称与 ID、Dataset、Agent Name、Harness、模型、任务总数、已完成、异常失败、得分、成本、Token 消耗、运行时长和创建时间，不设置 Job 状态列。已完成在同一单元格拆分为“通过 / 未通过”；异常失败只统计 Harbor `n_errored_trials`。终态 Job 满足“任务总数 = 已完成（通过 + 未通过）+ 异常失败”；运行中尚未终态的任务是剩余量，不得伪装成任一终态分类。列表存在 queued/running Job 时每秒刷新；running Job 名称前展示 loading 动画，运行时长从 `started_at` 起按秒持续更新，终态后固定为 `finished_at - started_at`。运行态任务计数读取当前 Job 独占的 Harbor 原生结果，不得因为数据库尚未写入终态 `result_path` 而显示为 0，也不得误读共享 `jobsDir` 中其他 Job 的旧结果。点击行打开可拖拽宽度的详情抽屉；抽屉展示事件、绝对日志路径、绝对产物路径和可展开 Trial 列表。Trial 只展示 Harbor 结果中可确认的状态、得分、成本、Token、时长、重试次数和日志路径；不存在的运行中百分比、分析路径或验证器内部字段不得伪造。
...
Job 详情为终态 Job 提供永久删除入口，删除前必须确认并列明影响。删除范围包括该 Job 的 Run、仅属于它且不再包含其他 Run 的 Experiment、队列状态、事件、私有配置、关联 Operation、排行榜资格，以及能够证明由该 Job 独占的 Harbor 输出、日志、报告和 Trial 产物。排行榜由 Run 派生，Run 删除后同步消失。queued/running Job 必须先取消；无法证明归属的共享父目录或外部文件不得删除，此时整体请求失败并保留数据库记录。Dataset、Agent、Environment、共享 `jobsDir` 及其他 Job 的产物永不随 Job 删除。
...
每个 Agent 模型独立选择成本来源：Harness 上报、LiteLLM 目录或自定义价格。自定义价格按每百万 Token 分别填写输入缓存未命中、输入缓存命中和输出单价。Harness 模式不展示任何固定单价，明确提示任务完成后展示 Harness 上报的总价格；LiteLLM 模式立即展示当前目录中的三项只读单价。创建 Job 时必须把本次模型的已解析价格保存为不可变快照；后续 Agent 编辑或 LiteLLM 目录更新不能改变历史 Job。Jobs、Trial 和 Leaderboard 使用同一快照与 Harbor Token 统计计算成本；原始 Harness 上报金额保留在 Harbor 结果中，不被 OrnnLab 改写。缓存命中/未命中价格不同时，如果 Harbor 没有提供缓存 Token，用 `-` 表示无法估算，不得默认为零命中。

Docker Agent 默认继承 OrnnLab 进程中的标准 `HTTP_PROXY`、`HTTPS_PROXY`、
`ALL_PROXY` 与 `NO_PROXY` 大小写变量。回环转换必须读取当前 Docker Context、有效
daemon endpoint、daemon OS 与 rootless 安全选项，按能力选择策略，不得把当前设备的
接口名、固定 IP、Docker Desktop、Colima、Clash 等产品特征写成通用假设。非回环代理
作为独立可寻址 URL 直接继承；只有确认 daemon 是同一 Linux 主机的 rootful Docker、
且 host gateway 可绑定时，才能把回环 HTTP/SOCKS 代理转换为 Job 生命周期内的临时
relay。Docker Desktop、rootless 或远程 daemon 等不能安全建立宿主 relay 的目标必须
在 Harbor 启动前明确失败，并提示使用容器可达的 Agent/Environment 代理。

Agent 或 Environment Profile 的显式代理配置必须在自动发现前按变量组裁剪并优先。
Agent 或 Environment 配置 `extra_allowed_hosts` 时，默认自动代理必须整体跳过，避免自动开放一个可绕过
目标白名单的通用出口；需要代理时由用户在 Profile 中显式配置并承担网络策略一致性。
不得因无关的宿主回环代理失败。用户可通过 `ORNNLAB_DOCKER_PROXY_MODE=off` 关闭自动
继承。自动值只属于 Agent 运行环境，不承诺配置 daemon image pull、Dockerfile build、
Verifier 或 sidecar。不得静默直连、记录代理凭据或让 relay 跨 Job 常驻。
...
Dataset 详情抽屉展示来源、本地路径、大小、Registry 与 Task 列表。Task 列表按名称搜索并由服务端分页，每页 20 条；搜索条件变化后回到第一页，不允许前端预取或平铺完整 Task 集合。不提供 Dataset 级 `split` 筛选，因为当前 Harbor Dataset contract 没有该通用配置。点击“运行单 Task”进入 New Job 并预选择该 Task，而不是伪造一个独立的单 Task 执行接口。

已下载或本地导入的 Dataset 列表接口只返回当前页的 Task 名称，不解析 `task.toml` 或环境定义；用户展开单个 Task 时才读取该 Task 的描述、运行环境摘要与镜像平台。在 Task 展开区展示操作系统、预构建镜像、镜像 Registry 声明的平台、Dockerfile/Docker Compose 定义、构建超时、网络模式、资源要求和工作目录。远程但未下载的 Registry Dataset 只展示 Registry 已提供的 Task 名称，并提示下载后可查看运行环境。v1.0.5 不判断本机兼容性、不构建替代镜像，也不因解析结果阻止 Job；镜像平台查询失败时只显示“未知”。
...
- 每个可选模型保存独立价格来源。Harness 上报保持兼容；LiteLLM 在 Job 创建时解析公共目录；自定义模式要求输入缓存未命中、输入缓存命中和输出三项非负价格。
...
- API 模式只访问 `/api/webui/v1`，mock 模式与 API 模式遵循相同的写操作约束；Job 永久删除同步返回删除结果，其他既有写操作保持 Operation 生命周期。
...
- Given Job 正在运行, when 用户停留在 Jobs 列表, then 每秒刷新任务计数与运行秒表，名称前显示 loading；Job 终态后 loading 消失、秒表停止，且任务总数等于已完成加异常失败。
