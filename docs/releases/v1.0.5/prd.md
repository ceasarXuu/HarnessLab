...
用户点击“新建 Job”进入二级页面 `/jobs/new`。页面按配置领域分组：基础、Tasks、环境资源、Verifier/运行时、Hub/Leaderboard。基础区保留高频 Agent/Harness 选择，task split、include/exclude 和数量限制进入 Tasks。用户确认配置后运行 Job，回到 Jobs 列表并打开新 Job 详情。
