...
每个 Agent 模型独立选择成本来源：Harness 上报、LiteLLM 目录或自定义价格。自定义价格按每百万 Token 分别填写输入缓存未命中、输入缓存命中和输出单价。Harness 与 LiteLLM 模式都必须立即展示当前 LiteLLM 目录中的三项只读单价；Harness 模式明确标注这些值只用于参考，最终金额仍取 Harness 运行时上报。创建 Job 时必须把本次模型的已解析价格保存为不可变快照；后续 Agent 编辑或 LiteLLM 目录更新不能改变历史 Job。Jobs、Trial 和 Leaderboard 使用同一快照与 Harbor Token 统计计算成本；原始 Harness 上报金额保留在 Harbor 结果中，不被 OrnnLab 改写。缓存命中/未命中价格不同时，如果 Harbor 没有提供缓存 Token，用 `-` 表示无法估算，不得默认为零命中。
...
- Given Agent 模型选择 Harness 上报或 LiteLLM 目录, when 编辑模型设置, then 展示输入缓存未命中、输入缓存命中和输出三项目录单价；Harness 模式同时说明实际成本以运行时上报为准，目录缺少模型时明确显示不可用。
