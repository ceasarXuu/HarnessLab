...
| `setup.run_as` | 否 | `root` / `harnesslab` / `current` | agent 命令在 sandbox 内的运行用户，默认 `harnesslab`。 |
...
`sandbox_setup_command` 这类任意 shell 只能作为高级逃生口，不应成为普通用户注册 agent 的主要方式。MVP 之后的 agent 注册体验应优先使用参数化 setup：
