...
- `tests/REQUIREMENTS.yaml` 是 `generate-test-traceability.sh` 的主输入；没有进入 manifest 的需求不算已被测试工程保护。
...
tests:
  - id: ORCH-003
    ...
    source:
      ...
      section: "5.4"
    verifies:
      - requirement: run_exit_code_priority
      - contract: TaskAttemptResult
    command: "scripts/test-after-change.sh --select ORCH-003"
    file_patterns:
      - "tests/unit/**/test_exit_code*"
    required_artifacts:
...
- Every test ID in `docs/mvp-development-spec.md` must appear in `tests/TEST_REGISTRY.yaml`.
- Every requirement in `tests/REQUIREMENTS.yaml` must be verified by at least one active registry entry.
...
- No skipped test is allowed in `tests/TEST_REGISTRY.yaml` with `status: active`.
...
- A secret appears in generated JSON/HTML/YAML/TXT artifacts.
...
| Check | Pass Standard |
|---|---|
| Registry skeleton | `tests/TEST_REGISTRY.yaml` validates against schema and contains all M0 IDs. |
| Requirement skeleton | `tests/REQUIREMENTS.yaml` validates against schema and covers all M0 requirements. |
...
2. Create `tests/TEST_REGISTRY.yaml` schema and registry verifier.
